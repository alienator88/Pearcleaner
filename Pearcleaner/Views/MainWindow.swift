...
struct MainWindow: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            // Helper required overlay
            HelperRequiredOverlay()
...
// MARK: - Helper Required Overlay

struct HelperRequiredOverlay: View {
    @EnvironmentObject var updater: Updater
    ...
    // Show overlay if: helper not installed OR helper required but missing (after dismissal)
    private var shouldShowOverlay: Bool {
        !helperManager.isHelperToolInstalled || helperManager.helperRequiredButMissing
    }
...
//                                Text("Deprecated authentication fallback has been removed.")
//                                    .font(.subheadline)
//                                    .foregroundStyle(ThemeColors.shared(for: colorScheme).secondaryText)
//                                    .fixedSize(horizontal: false, vertical: true)
                            ...
                            Button {
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                                    helperManager.dismissHelperRequiredAlert()
                                }
                            } label: {
                    ...
                    .padding(10)
                    .frame(width: isExpanded ? 410 : 410)
                    .background(
                        RoundedRectangle(cornerRadius: 12)
                            .fill(ThemeColors.shared(for: colorScheme).secondaryBG)
            ...
            .onChange(of: shouldShowOverlay) { newValue in
                withAnimation(animationEnabled ? .spring(response: 0.4, dampingFraction: 0.8) : .none) {
                    isVisible = newValue
                }
            }
            .onChange(of: helperManager.helperRequiredButMissing) { newValue in
                // When explicitly triggered, always show overlay
                if newValue {
                    withAnimation(animationEnabled ? .spring(response: 0.4, dampingFraction: 0.8) : .none) {
                        isVisible = true
                    }
