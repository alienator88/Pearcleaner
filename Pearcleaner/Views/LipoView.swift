...
struct LipoView: View {
    ...
    // Filter the apps to only include universal ones and exclude excluded apps
    var universalApps: [AppInfo] {
        appState.sortedApps.filter { $0.arch == .universal && !excludedApps.contains($0.path.path) }
    }
    ...
    var body: some View {
        ZStack {
            ...
            LipoSidebarView(infoSidebar: $infoSidebar, excludedApps: excludedApps, prune: $prune, onRemoveExcluded: removeAppFromExcluded, totalSpaceSaved: totalSpaceSaved, savingsAllApps: savingsAllApps)
        }
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            RoundedRectangle(cornerRadius: 4).fill(Color.clear)
                ...
                .padding(2)
                .overlay(
                    RoundedRectangle(cornerRadius: 4).strokeBorder(Color.secondary.opacity(0.5), lineWidth: 1),
                    alignment: .center
                )
                .overlay (
        ...
        if infoSidebar {
            HStack {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Spacer()
                    LipoPruneToggleSection(prune: $prune)
                }
                .padding()
                .frame(width: 280)
...
struct LipoPruneToggleSection: View {
    @Binding var prune: Bool
