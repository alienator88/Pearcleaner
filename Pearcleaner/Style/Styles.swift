...
// MARK: - Custom Menu Dropdown

struct CustomMenuDropdown<Item: Identifiable & Hashable, Label: View, ItemContent: View>: View {
            ...
            if isOpen {
                VStack(alignment: .leading, spacing: spacing) {
                    ForEach(items) { item in
                        ...
                        .contentShape(Rectangle())
                        .onHover { hovering in
                            // Hover effect handled by changing cursor
                            if hovering {
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 8)
                        .strokeBorder(
                            ThemeColors.shared(for: colorScheme).secondaryText.opacity(0.2),
                            lineWidth: 1
                        )
                )
                .frame(width: menuWidth)
                .offset(y: 32) // Position below button
